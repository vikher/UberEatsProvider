...
namespace ClubersProviderMobile.Prism.ViewModels
{
    public class CommandPageViewModel : ViewModelBase
    {
        ...
        public CommandPageViewModel(INavigationService navigationService, IApiService apiService, IPageDialogService dialogService) : base(navigationService)
        {
            Title = "Abiertas";
        ...
        private async void LoadOrdersAsync()
        {
            ...
            _myorders = response.Result.Where(x => x.ConsumptionTypeId == 3 && x.TrackingStatusId == 2).ToList();
            ShowOrders();
        }
        ...
        private async void OrderDetails(Orders obj)
        {
            ...
            if (SelectedOrder == null)
                return;
            if (SelectedOrder.PaymentId == 1)
            {
                var parameters = new NavigationParameters
                    {
                        { Constants.order, SelectedOrder }
                    };
                await _navigationService.NavigateAsync(nameof(CommandDetailPage), parameters);
            ...
            else
            {
                ...
                await _navigationService.NavigateAsync(nameof(ApproveDishPage), parameters);
            }
            SelectedOrder = null;
        }
    }
