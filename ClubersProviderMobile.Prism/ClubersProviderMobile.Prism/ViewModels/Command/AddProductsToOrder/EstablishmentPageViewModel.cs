...
{
    public class EstablishmentPageViewModel : ViewModelBase
    {
        ...
        public EstablishmentPageViewModel(INavigationService navigationService, IApiService apiService, IPageDialogService dialogService) : base(navigationService)
        {
        ...
        public override void OnNavigatedTo(INavigationParameters parameters)
        {
            ...
            else if (parameters.ContainsKey(Constants.product))
            {
                ...
                Products.Add(new OrderProduct() { Product = _selectedProduct, Comments = Comments, Quantity = Quantity});
                IsRunning = false;
            }
        ...
        private void ShowProducts()
        {
            ...
            {
                var sorted = from Product in _myproducts where Product.ProductSection.Any()
            ...
            {
                var sorted = from Product in _myproducts where Product.ProductSection.Any() && Product.Name.ToUpper().Contains(Search.ToUpper())
                             orderby Product.Name
                             group Product by Product.ProductSection.ToArray().LastOrDefault().Section.Name into ProductGroup
                             select new Grouping<string, Products>(ProductGroup.Key, ProductGroup);
                //create a new collection of groups
